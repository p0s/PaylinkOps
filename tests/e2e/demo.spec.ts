...
test('demo mode landing, dashboard, wallets, and receipts API work', async ({ page, request }) => {
  await page.goto('/');
  await expect(page.getByRole('heading', { name: 'PaylinkOps' })).toBeVisible();
  ...
  await expect(page.getByText('Installed: No')).toBeVisible();
  ...
  await expect(page.getByText('demo-treasury')).toBeVisible();
